...
\tikzstyle{randgradedgeh} = [draw,dash dot, thick, gradcolor, transform canvas={yshift=-0.5em}, font=\scriptsize ]
\tikzstyle{forwedge} = [draw, thick, black!50, font=\scriptsize]
\tikzstyle{randforwedge} = [draw,dash dot, thick, black!50, font=\scriptsize]
...
\begin{tikzpicture}[node distance = 1.5cm, auto]
    ...
    \node  (L1e) at (\sep*1,0) {};
    \node [loss, above of=L1e] (L2e) {$\mathcal{L}^{(1)}$};
		\node [loss, above of=L2e] (L3e) {$\mathcal{L}^{(2)}$};
  ...
  \path [->, randforwedge]  (x20) -- (L2e) ; %node[pos=.5] {$W^{(1,1)}$};
	\path [->, randforwedge]  (x30) -- (L3e) ; %node[pos=.5] {$W^{(2,2)}$};
  \path [->, gradedgeh] (L2e) edge (x20); % node[midway,below] {$\delta^1$};
  \path [->, gradedgeh] (L3e) edge (x30); % node[midway,below] {$\delta^1$};
