...
%    \begin{split}
%    x[t] = & \sigma(h[t]) \\
%    h[t+1] = & h[t] + \Delta t g(V h(t),{\bf W}x(t))
%    \end{split}
%    \]
...
% TODO the box needs a figure of the network
\begin{infobox}
  \begin{mdframed}[backgroundcolor=black!10]\small
    ...
    \begin{equation}\label{eq:bp_shallow}
      \begin{split}
        ...
        %\frac{\mathrm{\partial}}{\mathrm{\partial} w_{ij}} \mathcal{L} = \rho'\left(\sum_j w_{ij} x_j\right) e_i x_j,
        \Dp[\mathcal{L}]{W_{ij}} = 
       \Dp[\mathcal{L}]{y_i} %err_i
       \Dp[y_i]{ a_i }       %\rho'\left( a_i \right)
       \Dp[a_i]{W_{ij}}      %x_j  
        %     & = e_i \rho'\left(\sum_j w_{ij} x_j\right) x_j,0
      \end{split}
    \end{equation}
    %
...
%For a \gls{lif} neuron with first order dynamics, recall that the discrete-time membrane potential and synaptic current is expressed as Eqs.~\eqref{eq:syn_discrete_time} and \eqref{eq:mem_discrete_time}.
%\[
%\begin{split}
...
%%Emre: Following can move to the discussion
...
%%Emre: Add something about vanishing gradients and necessity for buffer in BPTT and eligibility
...
% TODO add Figure with different surrogate partials used
