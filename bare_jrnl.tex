...
For illustration purposes, we assume a neural network with $L$ layers and a cost function $\mathcal{L}[t] = \sum_i (U_i[t]-\hat{U_i}[t])^2$, using the notation $\delta_i[t]^{(L)} = U_i[t]-\hat{U_i}[t]$. 
