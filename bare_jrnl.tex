...
\begin{equation}\label{eq:bp_deep}
  \frac{\mathrm{\partial}}{\mathrm{\partial} W^{(l,l)}_{ij}} L = \sum_{t=0}^T \delta_{i}^{l}[t]  y^{l-1}_j[t],
    \text{ where }
    \delta_{i}^{l} [t] = \rho'\left(\sum_j W^{(l,l)}_{ij} y^{l}_j [t] \right) \sum_k \delta_{k}^{l+1}W_{ki}^{(l,l+1)},
\end{equation}
%
where $T$ is the duration of the sequence.
This rule is known as backpropagation through time (BPTT), and the main difference is the sum over $t$.
Using BPTT, the temporal credit assignment problem can be solved by back propagating through the unrolled network.
