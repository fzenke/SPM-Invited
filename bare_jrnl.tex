...
often constitute a viable solution CITE. % CITE review However, to deploy
\glspl{rnn} two key challenges need to be addressed.  First, before an \gls{rnn}
...
floating-point multiplications. % CITE paper on binary embedded nets \Glspl{snn}
form a special subset of \glspl{rnn} with a binary activation functions and
...
ie.\ units which are neither directly connected to the input nor the output of
the network.  However, the majority of real-world problems require hidden
units to be solved efficiently. To learn useful representations in hidden units
requires efficient solutions to the spatial and temporal credit assignment
problems.  Solving credit assignment in \glspl{snn} creates a set of specific
challenges, which need to be tackled.  In this article we focus on these
...
refer to in the following as the ``smoothed'' and \gls{sg} approach for brevity.
...
\cite{brea_matching_2013, rezende_stochastic_2014}.
...
% TODO need to sort  into above categories
\cite{OConnor_Welling16_deepspik} % TODO Help! Not sure this qualifies as SG need to
...
Decades of research in machine learning has endowed us with a rich repertoire of
...
\begin{equation}
    \tau_\mathrm{mem} \frac{dU_i}{dt} = -(U_i-U_\mathrm{rest}) + RI_i
    \label{eq:lif_basic}
\end{equation}
...
\begin{equation}
    \frac{dI_i}{dt}= -\frac{I_i(t)}{\tau_\mathrm{syn}} + \sum_j W_j S_j(t)
    \label{eq:cuba_syn}
\end{equation}
...
\begin{equation}
    \frac{U_i}{dt} = -\frac{1}{\tau_\mathrm{mem}}(U_i-U_\mathrm{rest}) + RI_i + S_i(t)(U_\mathrm{rest}-\vartheta)
    \label{eq:lif}
\end{equation}
...
\label{sec:temporal_credit_assigment}
...
\label{sec:spatial_credit_assigment}
...
\bibliography{biblio_unique_alt,fzenke}
