...
\begin{comment}
  Emre: The Turing discussion is too detailed here. Make it briefer. 
\end{comment}
...
\cite{OConnor_Welling16_deepspik} % TODO Help! Not sure I understand this paper and whether this qualifies as SG need to
...
1)~The recurrent neural network is ``unrolled'' meaning that a graph is created by copying the network for each time step of the system (see figure \ref{fig:snn_compgraph}). 
...
Thus the space complexity this approach is $O(N T)$, where $N$ is the number of neurons.\\
On the other hand, the forward method requires maintaining all the inputs and activations for each time step.
Thus the space complexity this approach is $O(N T)$, where $N$ is the number of neurons.\\
Thus, computing the weight update $\Delta_{w_i} \mathcal{L}$ requires maintaining variables $P_{ij}$.
The space complexity is $O(N^2)$, where $N$ is the number of neurons.
In a single layer feed-forward architecture, the third term above vanishes and $p_{ij}$ does not depend on $j$. In this case, the space complexity becomes $O(N)$. 

As memory is large in modern computers, the backward method is preferred for an implementation on a conventional computer.
On the other hand, processing in dedicated neuromorphic hardware and, more generally, non-von Neumann computers may not have immediate access to such memory.
...
\section*{Section to be reintroduced in body}
